...
def ProteinDataloaders(args):
    traincal_dataset = ProteinDataset(
        args.datapath + "/hierarchy_full.txt",
        args.datapath + "/proteins_train_tfidfoh.svm",
    )
    # now split in train and calibration 
    generator = torch.Generator().manual_seed(args.randomseed)
    train_dataset, cal_dataset = random_split(traincal_dataset, [1-args.calratio, args.calratio], generator=generator)
    ...
    return train_dataloader, cal_dataloader, test_dataloader, traincal_dataset.y
...
def BacteriaDataloaders(args):
    traincal_dataset = BacteriaDataset(
        args.dim,
        args.datapath + "/hierarchy_full.txt",
...
def CaltechDataloaders(args):
    ...
    traincal_dataset = CaltechDataset(args.datapath + "/TRAINVAL.csv", transform) 
    # now split in train and calibration 
    generator = torch.Generator().manual_seed(args.randomseed)
    train_dataset, cal_dataset = random_split(traincal_dataset, [1-args.calratio, args.calratio], generator=generator)
...
def PlantclefDataloaders(args):
    ...
    traincal_dataset = PlantclefDataset(args.datapath + "/TRAINVAL.csv", transform)
    # now split in train and calibration 
    generator = torch.Generator().manual_seed(args.randomseed)
    train_dataset, cal_dataset = random_split(traincal_dataset, [1-args.calratio, args.calratio], generator=generator)
