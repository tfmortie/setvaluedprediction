...
class SVPTransformer(TransformerMixin, BaseEstimator):
    ...
    def get_hstruct_tensor(self, params):
        ...
        ----------
        params : dict
            Represents the parameters for the SVP problem.

        Returns
        -------
        hstruct_t : tensor of size (S, K) with S the search space size and K the number of classes
        """
        hstruct_t = []
        rc = params["c"]
                ...
                if ci==1:
                    ...
                    if len(set(sum(list(c),[])))==len(sum(list(c),[])) and (set(sum(list(c),[])) not in [set(n) for n in self.hstruct_]):
                        if params["constraint"] == "size":
                            ...
                            crow[0,sum(c,[])] = 1
                            hstruct_t.append(crow)
        hstruct_t = torch.tensor(np.concatenate(hstruct_t,axis=0))
